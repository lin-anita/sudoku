...
int board_num;//用於儲存之後的隨機數

int set_board() 
{
	/*設定此次遊戲的題目是誰、產生此次遊戲的決定性隨機數*/
	// 使用當前時間作為亂數種子
    srand(time(NULL));
...
{
    /* Move the cursor up, down, to the left or to the right.
...
void print_board(int count/*main函式的while迴圈執行第幾次*/) 
{
    // Flush the screen
...
void initialize()
{
    ...
    for (int i = 0; i < 9; ++i)
        for (int j = 0; j < 9; ++j)
            editable[i][j] = !board[i][j];//原題空處為可編輯(1) 
